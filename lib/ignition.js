...
ignition.getInstanceDetails = function(instanceId, callback) {	
	ignition.describeInstances(instanceId, function(e, instance) {
		...
		var launchtime_string = launchtime ? moment(new Date(launchtime)).format("ddd Do, hA") : 'unknown';

		return callback(null, { currentState: state, action: stateText, name: name, launchtime: launchtime_string, health: 'unknown' });
	});
};
